...
AWS multi-account Terraform monorepo managed by **Terragrunt**. 39 module types under `iac-template-terraform/modules/aws/`, called by 504 `terragrunt.hcl` stacks across 50 stack directories under `iac-terragrunt/invictus/`. Azure DevOps CI/CD with external pipeline template.

## Critical: Placeholder substitution

All environment/account-specific values use `#{variable_name}#` syntax (e.g. `#{aws_region}#`, `#{aws_environment}#`, `#{aws_bucket}#`, `#{aws_cliente}#`). These are **resolved by the CI/CD pipeline** — the external template at `DevOps-templates-UX/pipelines-templates` (branch `feature/infrastructure-as-code`). Terragrunt/Terraform **never** resolves them. Running terragrunt locally without substitution will produce broken configs with literal placeholder strings.

## Layout

```
iac-template-terraform/modules/aws/   → 39 reusable Terraform modules (source of truth)
iac-terragrunt/invictus/              → 50 deployable stack directories
  ...
  {48 microservice directories}/      → per-service stacks (ECS, SSM, SQS, S3, IAM, etc.)
...
`root.hcl` auto-generates `provider.tf` and `backend.tf` in each directory via `generate` blocks. Do not edit these manually. `.gitignore` excludes local Terraform/Terragrunt artifacts (`**/.terraform/`, `*.tfstate*`, `*.tfvars`, `**/.terragrunt-cache/`).
...
- Only two docs at repo root: `README.md` (Spanish) and `AGENTS.md` (English).
...
- CloudWatch logs retention is set to 30d where configured (e.g. apigateway module).
- AWS provider pinned to `= 6.8.0` (exact pin in root.hcl).
