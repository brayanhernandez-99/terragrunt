...
inputs = {
  ssm_parameters = {
    ...
    CONCILIATION_FILE_MODALITIES = {
      type        = "String"
      name        = "/${upper(local.service)}/CONCILIATION_FILE_MODALITIES"
      value       = "{\"CMI\": \"01\"}"
      description = "Modalidades para el archivo de conciliacion de productos externos"
    }
    SCHEDULE_ROL = {
      type        = "String"
    ...
    CONCILIATION_BUCKET_NAME = {
      ...
      value       = "conciliation-${get_aws_account_id()}"
      description = "name bucket conciliation"
    }
    CEM_COLLECTION_NAME = {
      type        = "String"
      name        = "/${upper(local.service)}/CEM_COLLECTION_NAME"
      value       = "Archivos cem"
      description = "Parametro para la conexión entre metabase y la opcion de generar archivos"
    }
  }
}
