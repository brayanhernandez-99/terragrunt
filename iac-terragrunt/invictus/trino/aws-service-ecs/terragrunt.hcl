...
terraform {
  source                    = "${get_repo_root()}/iac-template-terraform/modules/aws/aws-service-ecs-trino"
}
...
inputs                        = {
  ...
  security_group_id_metabase  = dependency.vpc.outputs.ecs_metabase_security_group_id
  
  ecs_task                    = {
    cpu                       = "1024"
    memory                    = "2048"
    container_image           = "861262569826.dkr.ecr.us-east-1.amazonaws.com/trino-#{aws_trino_image_name}#:1.16.0",
