...
inputs = {
  ...
  ecs_task = {
    cpu    = "#{aws_cpu_transaction_lotteries_games}#"
    memory = "#{aws_memory_transaction_lotteries_games}#"
  ...
  target_group_config = {
    ...
    health_check_command  = ["CMD-SHELL", "curl -kf http://localhost:8080/health || exit 1"]
  ...
  ecs_service_autoscaling = {
    min_capacity        = "#{aws_autoscaling_min_capacity_transaction_lotteries_games}#"
    max_capacity        = "#{aws_autoscaling_max_capacity_transaction_lotteries_games}#"
