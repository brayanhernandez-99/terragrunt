...
dependency "s3"                  {
  config_path                    = "../../aws-service-s3/s3-dynamic-storage"
  mock_outputs                   = {
...
inputs = {
  ...
  alternate_domain_names         = ["#{aws_cloudfront_alternate_domain_dynamic_storage}#"]
  price_class                    = "PriceClass_All"
  comment                        = "Distribución de CloudFront"
  acm_certificate_arn            = "#{aws_arn_certificate}#"
  aws_cloudfront_tags            = {
    Name                         = "dynamic-storage",
    Project                      = "microFrontend-dynamic-storage"
  }
