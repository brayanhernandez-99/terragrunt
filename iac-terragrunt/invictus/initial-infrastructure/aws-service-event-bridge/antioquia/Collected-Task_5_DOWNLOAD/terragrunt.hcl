...
inputs = {
  region              = "#{aws_region}#"
  scheduler_name      = "972453b9-ac34-4495-892c-b5d56d3e1c4f-Collected-Task_5_DOWNLOAD"
  aws_account_id      = "${get_aws_account_id()}"
  description         = "Tarea automática 5 DOWNLOAD"
  iam_role_arn        = dependency.iam_role.outputs.role_arn
  kinesis_name        = "Collected"
  schedule_expression = "cron(*/54 23-23/1 * * ? *)"
  payload = {
    model = {
      idConfiguration = 5
      type            = "DOWNLOAD"
