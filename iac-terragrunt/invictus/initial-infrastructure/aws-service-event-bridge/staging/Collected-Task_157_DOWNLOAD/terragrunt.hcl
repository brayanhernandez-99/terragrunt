...
inputs = {
  scheduler_name      = "87513cb8-ec2e-4a40-bf78-79e8e0f23cea-Collected-Task_157_DOWNLOAD"
  aws_account_id      = "${get_aws_account_id()}"
  description         = "Tarea automática 157 DOWNLOAD"
  iam_role_arn        = dependency.iam_role.outputs.iam_role_arn
  kinesis_name        = "Collected"
  schedule_expression = "cron(*/55 22-23/1 * * ? *)"
  payload = {
    model = {
      idConfiguration = 157
      type            = "DOWNLOAD"
