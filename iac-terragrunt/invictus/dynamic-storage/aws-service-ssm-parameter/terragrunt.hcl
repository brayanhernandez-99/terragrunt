...
inputs                           = {
  ssm_parameters                 = {
    ...
    EXPIRATION_CACHE             = {
      type                       = "String"
      name                       = "/${upper(replace(local.service, "-", "_"))}/EXPIRATION_CACHE"
      value                      = "86400000"
    ...
    DYNAMIC_STORAGE_BUCKET_NAME  = {
      type                       = "String"
      name                       = "/${upper(replace(local.service, "-", "_"))}/BUCKET_NAME"
      value                      = "dynamic-storage-${get_aws_account_id()}"
      description                = "name bucket dynamic-storage"
    }
  }
}