...
inputs                     = {
  ssm_parameters           = {
    PRODUCER_BASE_URL      = {
      ...
      value                = "redis://${dependency.load_balancer.outputs.nlb_dns_name}:${dependency.ecs.outputs.listener_port}/"
    ...
    REDIS_PASSWORD         = {
      type                 = "String"
      name                 = "/${upper(local.service)}/REDIS_SECRET"
      value                = "#{parameter_redis_password}#"
      description          = "Nombre del secreto de la informacion de autenticacion con Redis"
    }
  }
}