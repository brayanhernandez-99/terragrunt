...
terraform {
  source = "${get_repo_root()}/iac-template-terraform/modules/aws/aws-service-ecs-redis"
}
...
inputs                        = {
  ...
  ecs_task                    = {
    cpu                       = "256"
    memory                    = "1024"
    container_image           = "861262569826.dkr.ecr.us-east-1.amazonaws.com/redis-stack-server-prd:latest"
  ...
  security_group_id           = dependency.vpc.outputs.ecs_redis_security_group_id
