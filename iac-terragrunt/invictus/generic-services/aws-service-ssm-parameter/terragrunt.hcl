...
inputs                             = {
  ssm_parameters                   = {
    ...
    SCHEDULE_ROL                   = {
      ...
      description                  = "Nombre del schedule rol Generic Services"
    }
    KINESIS_POLL_DELAY             = {
      type                         = "String"
    ...
    GENERIC_SERVICES_BUCKET_NAME   = {
      type                         = "String"
      name                         = "/${upper(replace(local.service, "-", "_"))}/BUCKET_NAME"
      value                        = "generic-services-documents-${get_aws_account_id()}"
      description                  = "name bucket generic-services"
    }
  }
}