...
dependency "sqs_lotteries_games_admin" {
  config_path = "../aws-service-sqs"
  mock_outputs = {
    queue_name = "mock-queue-name"
  }
}
...
inputs = {
  ...
  ecs_task = {
    ...
    image  = "861262569826.dkr.ecr.#{aws_region}#.amazonaws.com/microservice-lotteries-games-#{aws_container_image}#:latest"
    portMappings = [
      {
        containerPort = 8080
      }
    ...
    environment = [
      {
        ...
        value = dependency.sqs_transaction.outputs.queue_name
      },
      {
        name  = "SQS_LOTTERIES_GAMES"
        value = dependency.sqs_lotteries_games_admin.outputs.queue_name
      },
      {
        name  = "ENVIRONMENT"
